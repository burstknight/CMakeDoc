...
### 巨集除錯: `assert.h`
前面提到可以藉由一個程式執行完以後的回傳值，來判斷這個程式是否執行成功，所以我們在使用`ctest`做單元測試時，其中一種作法是把一個專案的程式碼編譯成一個可執行檔，然後使用這個可執行檔測試程式碼是否有BUG。

雖然這種做法在做單元測試時會比較簡單，可是在某些情況下可能沒辦法使用。比方說我們開發的專案是一個函式庫，這種情況下我們不太可能為了測試程式碼而把程式碼編譯成可執行檔。這個時候就會另外寫些測試程式，並且把這些測試程式編譯成可執行檔，`ctest`就可以使用這些可執行檔做單元測試。

在寫測試程式時，我們除了可以使用前面說的方式，透過程式的回傳來確認測試是否成功，還有可以使用標頭檔`assert.h`提供的巨集。這個標頭檔裡面定義了幾個好用的巨集，可以輔助我們程式除錯。稍後會在範例中使用這個標頭檔來寫測試程式，在這之前先說明怎麼使用，我們主要會使用這個標頭檔定義的巨集`assert()`來測試程式碼是否有BUG，使用方式如下:
```c
assert(<condition>);
```

使用上來說我們可以像在呼叫函式那樣使用巨集`assert()`，並且在裡面放入一個條件判斷`<condition>`。只要給予的條件判斷不成立，程式就會停止運作，然後顯示錯誤訊息。

這邊先給一個範例來說明怎麼使用。假設有一個變數`a`，這個變數的型態是`int`，某個演算法會使用到這個變數，而且我們預期在跑完這個演算法以後，這個變數的值必定大於10。也就是說，假如這個演算法沒寫錯，變數`a`的值一定會是大於10的整數。我們可以這樣使用`assert()`來判斷這個演算法是否正確執行:
```c
assert(a > 10);
```

只要這個演算法正確執行，變數`a`的值就會是大於10的整數，所以當我們使用`assert(a > 10);`時什麼事情都不會發生。可是假如這個演算法存在著某些BUG，導致變數`a`的值為小於等於10的整數時，整個程式會停止在`assert(a > 10);`這行，然後我們會在終端機上看到錯誤訊息，原因就在於條件判斷`a > 10`不成立。

### 新增單元測試: `add_test()`
當我們寫好測試程式以後，我們需要透過編輯`CMakeLists.txt`來註冊一個新的單元測試，`ctest`才會知道哪些可執行檔可以用來做單元測試。在`CMakeLists.txt`中，我們可以使用`add_test()`新增一個單元測試，使用方式如下:
```cmake
add_test(<test_name> <exe_file> [<args>])
```

這個指令有三個參數可以設定:
* `<test_name>`: 這個是用來設定一個單元測試的名稱。
* `<exe_file>`: 這個用來指定可執行檔的路徑
* `<args>`: 假如給予的可執行檔需要參數時，可以在這邊設定，而且如果有多個參數時，需要使用空格隔開。若可執行檔不需要參數時，就可以不用給予參數。

這裡要提醒一下，假如我們開發的專案中有多個單元測試時，每一個測試最好給予唯一的測試名稱。我們在使用`ctest`時才會知道有哪些測試已經做完，又或是在測試失敗時才能判斷專案中哪些程式碼有問題。

### 設定測試屬性: `set_tests_properties()`
有時候我們在測試程式碼時，只用回傳值判斷測試否正確可能還不夠，或許我們還需要使用一些額外的資訊才能確認這個測試是否成功。這個時候我們可以使用指令`set_tests_properties()`，我們可以透過這個指令追加測試屬性，這裡提供其中一種使用方式：
```cmake
set_tests_properties(<test_name> <exe_file> PROPERTIES PASS_REGULAR_EXPRESSION <pass_pattern>)
```

這個指令有幾個重點：
* `<test_name>`: 需要給予想追加測試屬性的測試名稱。
* `<exe_file>`: 用來設定測試程式的路徑。
* `PROPERTIES`: 這個必須大寫，而且一定要給。
* `PASS_REGULAR_EXPRESSION`: 這個必須大寫，主要意義告訴`cmake`這個追加的測試屬性會使用正規化表示法來判斷該測試是否成功。
* `<pass_pattern>`: 我們可以放入一個字串，預期這個測試可能會輸出某種特定格式的字串。

### 開啟單元測試: `enable_testing()`
接下來介紹目前最簡單的指令，也就是`enable_testing()`。這個指令的功能是要求`cmake`開啟單元測試，我們在透過`cmake`產生建置檔案以後，才能夠使用`ctest`測試專案。這個指令使用起來非常簡單，只要在專案根目錄的`CMakeLists.txt`中加入這個指令即可，這個指令完全不需要給予任何參數。

基本上只要有使用`add_test()`，就一定要使用`enable_testing()`，而且`enable_testing()`必須在`add_test()`前面才行。h此外，習慣上我們會在專案中額外建一個資料夾，這個資料夾通常會命名為`test`或`tests`，這個資料夾通常用來放置測試程式碼，而且在這個資料夾中也會放入`CMakeLists.txt`。為了保險起見，最好也要在這個檔案中加入`enable_testing()`。

### cmake的巨集： `macro()`
在C和C++中我們可以使用`#define`宣告出一個巨集，甚至還可以把巨集當函式來使用，讓我們可以簡化程式碼，也比較容易閱讀。在`cmake`中我們也可以使用巨集，把重複使用的指令整合起來，未來在管理上也比較容易。`macro()`這個指令就是用來宣告巨集的，使用方式如下：
```cmake
macro(<name> [<args>])
	<commands>
endmacro()
...
這個指令有幾個重點：
* 當我們宣告巨集時，必須用`macro()`當開頭，然後用`endmacro()`結尾。
* `<name>`: 這個是用來設定巨集的名稱。
* `<args>`: 如果這個巨集需要傳入參數，可以在這邊宣告好。
* `<commands>`: 這個是用來放入想執行的指令，如果有需要當然可以放多個指令。
